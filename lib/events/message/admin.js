...
const cancelMember = async (client, guild, message, member, minutes) => {
...
const createTimeout = (client, message, minutes = 5) => {
    ...
    if (userToCancel && guild) {
        ...
        if (userToCancel.id !== client.user?.id) {
            ...
            return cancelMember(client, guild, message, member, minutes);
        }
    } else {
